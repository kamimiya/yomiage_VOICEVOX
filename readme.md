...
5. 機能の追加、バグの修正などしていくつもりなのでよろしくお願いします。更新したらtwitterで報告します。
...
### すぐ(一週間以内)に追加予定
- 辞書機能の改善。現在、例えばpen="ペン"と登録されているときにpencil="鉛筆"を追加してもpencil->ペンcilとなってしまう。その改善
- ボイスチャンネル内に誰もいなくなったら自動で抜けるようにする。
- 時報ボイスの変更ができるようにしたい。
### 未定

- 各サーバーでつくられたカスタム絵文字の読み上げには対応しているが、デフォ絵文字の読み上げには対応していない。<br>
  →ユニコード絵文字になっているみたいなので、文字コードまわりの設定をきちんとする必要があるかと思う。
- 英字が連続した時に文章を省略する機能が欲しい
- 一部コードがとても汚いので改善したい。（特にon_messageのif分岐がひどい）
- exeファイル化して導入の手間を減らしたい。
- !stop機能の強化
  現在、2個以上チャットが流れている時、それを消化しなければ!stop機能が機能しない。<br>
  音声が再生されている時に!stop以外のチャットが流れたらwaitするため。<br>
  →音声が再生されている時にチャットが流れてきたらwaitするのではなく、それをBOTの内部情報に記録しておき、それをキュー処理するように変更する？そして!stopが流れてきたらその内部情報を初期化する。

## かなしみ欄

- discord.py開発終了してるらしい…。つくってから気がついた…。せっかく勉強したのに・・・。
  フォークプロジェクトが出てきてくれるとは信じていますが・・・。
　万が一の場合に備えて今一からjavascript勉強しておきます。discord.jsの方は大丈夫そうなので・・・。
- 2022年4月からmessage.contentが特権インテントになるらしい。
  これからはスラッシュコマンドしか読み取れなくなるとか何とか。
